...
`amber-svc-router-gate` is centered on backend services work in Kotlin. The technical target is: Design a Kotlin verification harness for router systems, covering stream reduction, windowed input fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
